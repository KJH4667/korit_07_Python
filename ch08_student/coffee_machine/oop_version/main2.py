...
while is_on:
    ...
    if choice == 'off':
        ...
        # if choice not in ['espresso', 'cappucino', 'latte']:
        #     continue
        drink = menu.find_drink(choice)
        if drink == None:       # choice에 오타가 있을 경우 None이 반환되기 때문에 고전적 예외 처리
            continue        # continue가 실행되면 이 이하의 코드라인은 실행되지 않고, while 반복문의 가장 앞 부분으로 돌아가서 다음 반복이 실행됨.
        ...
        if coffee_maker.is_resource_sufficient(drink):
            ...
            if money_machine.make_payment(drink.cost):
                coffee_maker.make_coffee(drink.cost)