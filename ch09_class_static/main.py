...
'''
객체명.클래스 변수를 통해서 클래스 변수에 접근이 가능하긴 하지만, 클래스 내부의 코드르 들여다보기 전까지는 country 라는 변수가 인스턴스 변수인지 클래스 변수인지 알 방법이 없다는 문제가 있습니다. 
이상을 이유로 클래스 변수를 확인하고자 할때는 객체명.클래스변수명 보다는 클래스명.클래스변수명 을 통해서 참조하는것이 권장됩니다. 

2. 크래스 메서드 : 클래스 변수를 사용하는 메서드 
'''
...
class Korean2:
    ...
    def trip(cls, travelling_site): # 그 결과 첫번째 매개변수가 self가 아니라 cls
       if cls.country == travelling_site: # 여기에 무조건 == 써야되네? 안쓰면 빨간줄 뜬다;;
           print('국내 여행 입니다.')
       else:
           print('해외 여행 입니다.')
...
man2 = Korean2()
man2.trip('일본')
# 클래스 변수와 마찬가지로 객체명. 클래스 메서드명() 으로 호출이 가능하기는 하지만 마찬가지로 이게 인스턴스 메서드인지 알바가 아니기 때문에 클래스메서드를 호출할때는 클래스명.클래스메서드명()으로 하는것이 권장됩니다.
...
3. 정적 메서드(static method)
    : 정적 메서드 또한 self를 사용하지않음 -> 즉, 인스턴스 변수에 접그하여 사용하는 것이 불가능함을 의미. self. 속성명을 사용하여 인스턴스 변수의 값을 참조하는데 정적 메서드는 아예 첫번째 매개변수가 고정이 아닙니다.
    - 클래스 메서드와의 공통점 # 1
...
이상을 토대로, 정적 메서드는 self / cls를 둘다 사용하지 않기때문에 인스턴스 / 클래스 변수를 모두 사용하지 않는 메서드를 정의하는 경우에 적합 합니다. 정작 메서드 자체는 클래스에 소속돼있지만 인스턴스에는 영햐을 주지도 않고 받지도 않습니다. 
...
가방을 만들때마다 현재 만들어진 가방이 몇 개인지 계산할수 있는 Bag클래스를 정의할 겁니다.
'''
# 클래스 정의
class Bag:
    # 클래스 변수의 선언 및 초기화
    ...
    def __init__(self): # 생성자 호출 및 인스턴스 변수를 정의할 용도. 그럼 생성자도 인스턴스 변수라고 할수 있겠네요(self가 있으니까)
        Bag.count += 1  # 생성자가 호출될때마다 (=객체가 생성될 때마다) 클래스 변수인 count가 1씩 증가함. cls.count가 아니라 클래스명.count 라는 것에 주목해야 합니다.
...
2. man 과 woman 인스턴스가 생성되면 다음과 같은 메시지를 출력할수 있도록 작성하시오.
김일이(가) 태어났습니다
김이이(가) 태어났습니다

3. 다음 코드를 통해서 전체 인구수를 조회할수 있도록 작성하시오.
print(f'전체 인구수: {Person.get_population()}')

4. 다음과 같은 명령어로 man 인스턴스를 삭제하시오.
del man 

5. man 인스턴스가 삭제되면 다음과 같은 메시지를 출력할수 있도록 소멸자를 정의하시오 
RIP김일
'''
class Person:
    # 클래스 변수의 선언 및 초기화
...
def get_population(cls):
    return cls.populatiuon


def __init__(self, name):
    self.name = name
    Person.population += 1# 인스턴스 메서드를 통해서 클래스 변수를 변화 시킨 거니까 클래스명. 클래스
    ...
    def sales(cls, menu_name, quantity):
        for menu_dict in cls.menu_list:
            if menu_name in menu_dict:  # 딕셔너리 내에 pair 하나밖에 없는 상태인데, 있으면 이하의 코드라인이 실행되고 아니면 넘어갈겁니다. 그러면 다음 반복으로 가겠네요.
                # in -> element를 기준 해야 하기 때문에 dictionary 의 element 중 'key'를 기준
                # 애체오 key가 없으면  value를 조회 못하는게 dictionary의 특징중 하나
                cls.total += menu_dict[menu_name] * quantity
                print(f'{menu_name}을 {quantity}개 판매')
