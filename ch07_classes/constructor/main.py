'''
아까 전에는 일종의 setter를 활용하여 속성에 속성값을 넣어줬습니다. 그림 Java에서 수업한 것처럼 속성값이 대입되지 않은 객체를 생성한 다음에 속성값을 집어넣어주는 과정을 거쳐야 합니다.
...
'''
이게 굳이 속성값에 대한 제한이 있지 않다면 빈 객체 만들어놓고 거기에 값 대입하는게 비효율적으로 느껴집니다. 그래서 저희는 생성자를 도입할겁니다.
...
'''
굳이 소멸자를 학습하는 이유 -> 객체가 생성되면 메모리 공간을 차지해서, 객체가 호출될때마다 그곳에서 불려나오게 됩니다. 
...
1. 다음 지시 사항을 읽고 이름을 저장하는 Person클래스를 생성하시오.
...
james is dead.
emily is dead.
라고 결과값이 나옵니다. 이 이유는 모든 코드블럭이 다 읽어지면 메모리에 할당된 객체는 자동소멸하기 때문입니다. 그래서 강제로 emily is dead. 를 출력하고 싶지 않다면 꼼수가 좀 필요합니다. 
'''
...
setter는 call2() -> 매개변수 있고 리턴은 없었다.
getter는 call3() -> 매개변수가 없고 리턴은 있었다. 
'''
class Student:
    # setter 예시
...
이상의 속성에 맞는 getter 도 추가하시오.

student1 객체를 생성하고,
김일, 20, 4.5를 각각 이름/나이/점수에 대입하시오.

getter만을 활용하여, 
김일 학생의 나이는 20살로, 파이썬 과목의 점수는 4.5점 입니다. 라고 출력하세요. 
'''